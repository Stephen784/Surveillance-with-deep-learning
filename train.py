...
def TrainModel(X_train: object, model: object, ckpt_name: object, hist_name: object, ckpt_path: object, epochs: object,
    ...
    History: object = model.fit(X_train, X_train, batch_size=batch_size, epochs=epochs, validation_split=0.1,
    callbacks=[ckpt, e_stop])
...
def main(config_gpu=True):
    # Setting up the GPU to avoid V RAM and other conflicts.
    ...
    for name in data:
        ...
        history.append(hist)
    print('\n---------- Model Trained on All Datasets Successfully! ----------')
